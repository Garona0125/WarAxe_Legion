...
class spell_mage_water_elemental_le : public SpellScript
{
	PrepareSpellScript(spell_mage_water_elemental_le);

	void HandleCast()
	{
		if (Unit* caster = GetCaster())
		{
			caster->CreateTamedPetFrom(78116, 31687);
			caster->GetCurrentSpell(1)->SendSpellCooldown();
			caster->GetCurrentSpell(1)->cancel();
		}
	}

	void Register() override
	{
		BeforeCast += SpellCastFn(spell_mage_water_elemental_le::HandleCast);
	}
};
...
void AddSC_mage_spell_scripts()
{
	...
	new spell_mage_water_elemental_le();
}
