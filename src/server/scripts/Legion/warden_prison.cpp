...
public:
    ...
    struct npc_q39694AI : public ScriptedAI
    {
        ...
        void Reset() override
        {
			//me->SetFlag(UNIT_FIELD_FLAGS, 33344);
            summons.DespawnAll();
            events.Reset();
        }
...
class npc_97273 : public CreatureScript
{
public:
	npc_97273() : CreatureScript("npc_97273") {}

	CreatureAI* GetAI(Creature* creature) const override
	{
		return new npc_97273AI(creature);
	}
	struct npc_97273AI : public ScriptedAI
	{
		npc_97273AI(Creature* creature) : ScriptedAI(creature) {}

		void MoveInLineOfSight(Unit* who) override
		{
			if (who->GetDistance(me->GetPositionX(), me->GetPositionY(), me->GetPositionZ()) < 50.0f)
			{
				if (who->GetTypeId() == TYPEID_UNIT && who->GetEntry() == 96783)
				{
					if (who)
					{
		...
		void DamageTaken(Unit* attacker, uint32& damage, DamageEffectType dmgType) override
		{
			if (attacker && attacker->GetEntry() == 96783)
			{
				damage = urand(1000, 2000);
			}
		}
	};
